//
// Created by David Ensikat on 05/6/2023.
//
...
struct resolver_entity* resolver_create_new_entity_for_var_node_custom_scope(struct resolver_process* process, struct node* node, int offset, struct resolver_scope* scope, void* private) {
  ...
  entity->scope = scope;
  assert(entity->scope);
  entity->dtype = node->var.type;
  entity->node = node;
  entity->name = node->var.name;
  entity->offset = offset;
  return entity;
}

struct resolver_entity* resolver_create_new_entity_for_var_node(struct resolver_process* process, struct node* node, int offset, struct resolver_scope* scope, void* private) {
  return resolver_create_new_entity_for_var_node_custom_scope(process, node, offset, resolver_scope_current(process), private);
}
struct resolver_entity* resolver_new_entity_for_var_node_no_push(struct resolver_process* process, struct node* node, int offset, struct resolver_scope* scope, void* private) {
  struct resolver_entity* entity = resolver_create_new_entity_for_var_node_custom_scope(process, node, offset, resolver_scope_current(process), private);
...
struct resolver_entity* resolver_new_entity_for_var_node(struct resolver_process* process, struct node* node, int offset, void* private) {
  struct resolver_entity* entity = resolver_new_entity_for_var_node_no_push(process, node, offset, resolver_scope_current(process), private);
...
struct resolver_entity* resolver_make_entity(struct resolver_process* process, struct resolver_result* result, struct datatype* custom_dtype, struct node* node, struct resolver_entity* guided_entity, struct resolver_scope* scope) {
  ...
  switch (node->type) {
	case NODE_TYPE_VARIABLE:
	  entity = resolver_new_entity_for_var_node_no_push(process, node, offset, scope, NULL);
	  break;
	default:
...
struct resolver_entity* resolver_get_entity_in_scope_with_entity_type(struct resolver_result* result, struct resolver_process* resolver, struct resolver_scope* scope, const char* entity_name, int entity_type) {
  // Are we accessing a struct or union
...
void resolver_build_function_call_arguments(struct resolver_process* resolver, struct node* argument_node, struct resolver_entity* root_func_call_entity, size_t* total_size_out) {
  if (is_argument_node(argument_node)) {
	resolver_build_function_call_arguments(resolver, argument_node->exp.left, root_func_call_entity,
										   total_size_out);
	resolver_build_function_call_arguments(resolver, argument_node->exp.right, root_func_call_entity,
										   total_size_out);
...
struct resolver_entity* resolver_follow_function_call(struct resolver_process* resolver, struct node* node, struct resolver_result* result) {
  ...
  // push function call entity to stack
...
struct resolver_entity* resolver_follow_unsupported_node(struct resolver_process* resolver, struct node* node, struct resolver_result* result) {
  bool followed = false;
  switch (node->type) {
	case NODE_TYPE_UNARY: resolver_follow_unsupported_unary_node(resolver, node, result);
	  followed = true;
	  break;
	default:
	  followed = false;
...
void _resolver_merge_compile_times(struct resolver_process* resolver, struct resolver_result* result) {
  ...
  while (1) {
	...
	struct resolver_entity* left_entity = resolver_result_pop(result);
	if (!right_entity) { // nothing to merge
	  break;
	}

	if (!left_entity) { // only one entity
	...
	// Left entity goes back to result, as we may be able to
	// merge with next entities
...
void resolver_finalize_result_flags(struct resolver_process* resolver, struct resolver_result* result) {
  ...
  // Need to iterate through all the results
  ...
  while (entity) {
	if (entity->flags & RESOLVER_ENTITY_FLAG_DO_INDIRECTION) {
	  // Load address of first entity, since we have indirection
  ...
  if (last_entity->dtype.flags & DATATYPE_FLAG_IS_ARRAY
      && (!does_get_address && last_entity->type == RESOLVER_ENTITY_TYPE_VARIABLE && !(last_entity->flags & RESOLVER_ENTITY_FLAG_USES_ARRAY_BRACKETS))) {
	flags &= ~RESOLVER_RESULT_FLAG_FINAL_INDIRECTION_REQUIRED_FOR_VALUE;
