//
// Created by David Ensikat on 30/4/2023.
//
...
static void expect_op(const char *op) {
  ...
  if (!next_token || next_token->type != TOKEN_TYPE_OPERATOR || !S_EQ(next_token->sval, op)) {
	compiler_error(current_process, "Expecting operator %s, but something else was provided", next_token->sval);
  }
}
...
void parse_single_token_to_node() {
  ...
  switch (token->type) {
	case TOKEN_TYPE_NUMBER: node = node_create(&(struct node){.type=NODE_TYPE_NUMBER, .llnum=token->llnum});
	  break;
	case TOKEN_TYPE_IDENTIFIER: node = node_create(&(struct node){.type=NODE_TYPE_IDENTIFIER, .sval=token->sval});
	  break;
	case TOKEN_TYPE_STRING: node = node_create(&(struct node){.type=NODE_TYPE_STRING, .sval=token->sval});
	  break;
	default: compiler_error(current_process, "This is not a token that can be converted to a node");
  }
}
...
void parse_exp_normal(struct history *history) {
  const struct token *op_token = token_peek_next();
...
void parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token,
...
void parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token,
  ...
  if (!parser_datatype_is_secondary_allowed_for_type(datatype_token->sval) && datatype_secondary_token) {
	compiler_error(current_process,
				   "You're not allowed a secondary datatype here for given datatype %s\n",
				   datatype_token->sval);
  }
...
void parser_datatype_init_type_and_size(
  ...
  switch (expected_type) {
	case DATA_TYPE_EXPECT_PRIMITIVE:
	  parser_datatype_init_type_and_size_for_primitive(datatype_token,
													   datatype_secondary_token,
													   datatype_out);
	  break;
	case DATA_TYPE_EXPECT_STRUCT: datatype_out->type = DATA_TYPE_STRUCT;
	  ...
	  break;
	case DATA_TYPE_EXPECT_UNION: compiler_error(current_process, "Unions unsupported\n");
	  break;
	default: compiler_error(current_process, "BUG: Unsupported datatype expectation");
  }
}
...
void parser_datatype_init(struct token *datatype_token,
						  ...
						  int pointer_depth,
						  int expected_type) {
  parser_datatype_init_type_and_size(datatype_token,
									 datatype_secondary_token,
									 datatype_out,
									 pointer_depth,
									 expected_type);
  ...
  if (S_EQ(datatype_token->sval, "long") && datatype_secondary_token && S_EQ(datatype_secondary_token->sval, "long")) {
	compiler_warning(current_process,
					 "Our compiler does not support 64-bit longs, therefore long long is defaulting to 32 bits\n");
	datatype_out->size = DATA_SIZE_DWORD;
  }
}
...
void parse_datatype_type(struct datatype *dtype) {
  ...
  if (datatype_is_struct_or_union_for_name(datatype_token->sval)) {
	if (token_peek_next()->type == TOKEN_TYPE_IDENTIFIER) {
	  ...
	  // Struct has no name, so handle it
...
void parser_ignore_int(struct datatype *dtype) {
  ...
  if (!parser_is_int_valid_after_datatype(dtype)) {
	compiler_error(current_process, "You provided secondary type int, not supported with current abbreviation\n");
  }
...
void parser_finalize_body(struct history *history,
  ...
  // Want the size of the largest node in the union
...
void parse_body_single_statement(size_t *variable_size, struct vector *body_vec, struct history *history) {
  ...
  // Change variable size, by size of statement node
...
void parse_body(size_t *variables_size, struct history *history) {
  ...
  if (!variables_size) {
	variables_size = &tmp_size;
  }

  struct vector *body_vec = vector_create(sizeof(struct node *));
  if (!token_next_is_symbol('{')) {
	parse_body_single_statement(variables_size, body_vec, history);
	parser_scope_finish();
	return;
  }
...
void parse_struct_or_union(struct datatype *dtype) {
  switch (dtype->type) {
	case DATA_TYPE_STRUCT: parse_struct(dtype);
	  break;
	case DATA_TYPE_UNION: break;
	default: compiler_error(current_process, "Provided datatype is not structure or union\n");
  }
}
...
void parse_variable_function_or_struct_union(struct history *history) {
  ...
  if (name_token->type != TOKEN_TYPE_IDENTIFIER) {
	compiler_error(current_process, "Expecting a valid name for given variable declaration\n");
  }
...
int parse_expressionable_single(struct history *history) {
  ...
  int res = -1;
  switch (token->type) {
	case TOKEN_TYPE_NUMBER: parse_single_token_to_node();
	  res = 0;
	  break;

	case TOKEN_TYPE_IDENTIFIER: parse_identifier(history);
	  res = 0;
	  break;

	case TOKEN_TYPE_OPERATOR: parse_exp(history);
	  break;
...
int parse_next() {
  ...
  switch (token->type) {
	...
	case TOKEN_TYPE_STRING: parse_expressionable(history_begin(0));
	  break;
	case TOKEN_TYPE_KEYWORD: parse_keyword_for_global();
	  break;
	case TOKEN_TYPE_SYMBOL: parse_symbol();
	  break;
  }
  return 0;
}
