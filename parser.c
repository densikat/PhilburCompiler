...
void parse_single_token_to_node() {
  ...
  switch (token->type) {
	...
	default: compiler_error(current_process, "This is not a single token that can be converted to a node");
  }
}
...
void parser_datatype_init_type_and_size(struct token *datatype_token,
  ...
  switch (expected_type) {
	...
	case DATA_TYPE_EXPECT_STRUCT: datatype_out->type = DATA_TYPE_STRUCT;
	  ...
	  break;
	case DATA_TYPE_EXPECT_UNION: datatype_out->type = DATA_TYPE_UNION;
...
void make_variable_node(struct datatype *dtype, struct token *name_token, struct node *value_node) {
  ...
  if (var_node->var.type.type == DATA_TYPE_STRUCT && !var_node->var.type.struct_node) {
	...
	fixup_register(parser_fixup_sys,
				   &(struct fixup_config){.fix = datatype_struct_node_fix, .end = datatype_struct_node_end, .private = private});
  }
}
...
void parser_scope_offset_for_stack(struct node *node, struct history *history) {
  ...
  variable_node(node)->var.aoffset =
	  offset + (upward_stack ? variable_node(node)->var.padding : -variable_node(node)->var.padding);
}
...
void parse_struct_or_union(struct datatype *dtype) {
  switch (dtype->type) {
	...
	case DATA_TYPE_UNION: parse_union(dtype);
	  break;

	default: compiler_error(current_process, "COMPILER BUG: The provided datatype is not a structure or union\n");
  }
}
