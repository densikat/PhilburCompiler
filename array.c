...
size_t array_brackets_calculate_size_from_index(struct datatype *dtype, struct array_brackets *brackets, int index) {
  ...
  struct node *array_brackets_node = vector_peek_ptr(array_vec);
  if (!array_brackets_node) {
	return 0;
  }

  while (array_brackets_node) {
	assert(array_brackets_node->bracket.inner->type = NODE_TYPE_NUMBER);
	int number = array_brackets_node->bracket.inner->llnum;
	size *= number;
	array_brackets_node = vector_peek_ptr(array_vec);
  }
